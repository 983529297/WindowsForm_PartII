...
namespace DrawingModel
{
    public class GoogleDriveService
    {
        ...
        /// <param name="uploadProgressEvent"> 上傳進度改變時呼叫的函式</param>
        /// <param name="responseReceivedEventHandler">收到回應時呼叫的函式 </param>
        /// <returns>上傳成功，回傳上傳成功的 Google Drive 格式之File</returns>
        public Google.Apis.Drive.v2.Data.File UploadFile(string uploadFileName, string contentType, Action<IUploadProgress> uploadProgressEvent = null, Action<Google.Apis.Drive.v2.Data.File> responseReceivedEventHandler = null)
        {
            ...
            return Upload(uploadProgressEvent, insertRequest, responseReceivedEventHandler, uploadStream).ResponseBody;//insertRequest.ResponseBody;
        }

        //upload
        private FilesResource.InsertMediaUpload Upload(Action<IUploadProgress> uploadProgressEvent, FilesResource.InsertMediaUpload insertRequest, Action<Google.Apis.Drive.v2.Data.File> responseReceivedEventHandler, FileStream uploadStream)
        {
            if (uploadProgressEvent != null)
                insertRequest.ProgressChanged += uploadProgressEvent;
        ...
        /// <param name="downloadProgressChangedEvent">當下載進度改變時，呼叫這個函式</param>
        public void DownloadFile(Google.Apis.Drive.v2.Data.File fileToDownload, string downloadPath, Action<IDownloadProgress> downloadProgressChangedEvent = null)
        {
            const string SPLASH = @"\";
        ...
        public Google.Apis.Drive.v2.Data.File UpdateFile(string fileName, string fileId, string contentType)
        {
            ...
            try
            {
                ...
                FilesResource.UpdateMediaUpload request = GetUpdate(file, fileId, stream, contentType);//_service.Files.Update(file, fileId, stream, contentType);
        ...
        //getupdate
        private FilesResource.UpdateMediaUpload GetUpdate(Google.Apis.Drive.v2.Data.File file, string fileId, MemoryStream stream, string contentType)
        {
            return _service.Files.Update(file, fileId, stream, contentType);
        }
    }
}
